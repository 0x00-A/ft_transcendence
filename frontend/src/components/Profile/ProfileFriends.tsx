...
const ProfileFriends = () => {
  ...
  const {
    ...
    error: onlineError,
  } = useGetData<Friends[]>(API_GET_ONLINE_FRIENDS_URL);
  const {
    data: offlineFriends,
    isLoading: isOfflineLoading,
    error: offlineError,
  } = useGetData<Friends[]>(API_GET_OFFLINE_FRIENDS_URL);
  ...
  return (
    <div className={css.profileFriendsContainer}>
      <div className={css.friendsHeader}>
        ...
        <button className={css.viewMore} onClick={() => navigate('/friends')}>
          {t('Profile.friends.viewMore')}
        </button>
      </div>
        ...
        {isBtnActive ? (
          isOnlineLoading ? (
            ...
            <div className={css.friendList}>
              {onlineFriends && onlineFriends.length == 0 && (
                <span className={css.noCurrentFriend}>
                  {t('Profile.friends.errors.noOnlineFriends')}
                </span>
              )}
              {onlineFriends &&
                onlineFriends.length > 0 &&
                onlineFriends?.map((friend: Friends) => (
                  <div className={css.friendItem} key={friend.id}>
                    <img
          ...
          <div className={css.friendList}>
            {offlineFriends && offlineFriends.length == 0 && (
              <span className={css.noCurrentFriend}>
                {t('Profile.friends.errors.noOfflineFriends')}
              </span>
            )}
            {offlineFriends &&
              offlineFriends.length > 0 &&
              offlineFriends?.map((friend: Friends) => (
                <div className={css.friendItem} key={friend.id}>
                  <img
                    src={friend.profile.avatar}
                    alt={friend.username}
