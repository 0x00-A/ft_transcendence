...
let incS = 0;
function drawPaddle(ctx, { x, y, w: width, h: height }, isLeftPaddle) {
    ctx.beginPath();
    if (isLeftPaddle) {
        ...
        ctx.arcTo(x + width, y + height, x + width - cornerRadius, y + height, cornerRadius); // Bottom-right corner
        // Bottom edge
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (ctx && canvas) {
        ...
        const resizeCanvas = () => {
            var _a;
            const rect = (_a = canvas.parentNode) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect();
            // Get the computed styles of the parent to calculate border width
            const computedStyle = window.getComputedStyle(canvas.parentNode);
        ...
        let ball = new Ball(canvas.width / 2 - 7, canvas.height / 2 - 7, ballRaduis);
        let paddle1 = new Paddle(10, canvas.height / 2 - pH / 2, pW, pH, 5, 0);
        let paddle2 = new Paddle(canvas.width - 10 - pW, canvas.height / 2 - pH / 2, pW, pH, 5, 0);
        const resetGame = () => {
            resetBall();
        ...
        function isCollidingWithPaddle(ball, paddle) {
            ...
            const withinHorizontalRange = ball.x + ball.raduis >= paddle.x &&
                ball.x - ball.raduis <= paddle.x + paddle.w;
            // Check if the ball is within the vertical range of the paddle
            const withinVerticalRange = ball.y + ball.raduis >= paddle.y &&
        ...
        const handlePaddleCollision = (ball, paddle) => {
            ...
            if (ballFromLeft || ballFromRight) {
                ...
                const relativeImpact = (ball.y - (paddle.y + paddle.h / 2)) / (paddle.h / 2);
        ...
        const player1AI = (ball, paddle, s = 12) => {
            if (ball.vx < 0) {
        ...
        const player2AI = (ball, paddle, s = 12) => {
            if (ball.vx > 0) {
                if (ball.y < paddle.y + paddle.h / 2)
                    paddle.y -= s;
                if (paddle.y <= 0)
